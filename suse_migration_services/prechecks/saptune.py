...
def check_saptune(migration_system=False):
    ...
    if not os:
        ...
        return  # no migration hint - non critical
    
    # Get pattern.
    patterns = _get_installed_patterns(path_prefix)
    
    # Get enabled status of sapconf and saptune.
    ...
    if os == "SLES":
        ...
        lookup = {'saptune:not-found sapconf:not-found': no_tuning_tuple,
                  ...
                  'saptune:enabled sapconf:enabled': base_tuning_tuple,  # should not happen and captured later
        ...
        log.warning('Could not determine current operating system!')
        return  # no migration hint - non critical
    ...
    try:
        ...
        prediction, marker_content = 'Cannot predict migration outcome!', None
    ...
    log.info(f'sapconf.service enabled: {sapconf_enabled_state}') 
    log.info(f'saptune.service enabled: {saptune_enabled_state}')
    if current_state == 'saptune:enabled sapconf:enabled':
        log.error('Both sapconf and saptune are enabled! This is a misconfiguration and need to be resolved before migrating!')
        return  # migration stooper!
    log.info(f'Migration outcome: {prediction}')

    # Write marker
    if migration_system and marker_content:
        _write_marker(marker_content, path_prefix=path_prefix)
...
def _get_service_enabled_state(service: str, path_prefix: str='') -> str:
    ...
    return 'enabled' if os.path.exists(service_link) else 'disabled'


def _write_marker(marker_content: str, path_prefix: str='') -> None:
